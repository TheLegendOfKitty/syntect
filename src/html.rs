...
///
/// ```
/// use syntect-patched::html::{ClassedHTMLGenerator, ClassStyle};
...
impl<'a> ClassedHTMLGenerator<'a> {
    ...
    pub fn parse_html_for_line_which_includes_newline(&mut self, line: String) -> Result<(), Error>{
        let parsed_line = self.parse_state.parse_line(line.clone(), self.syntax_set)?;
    ...
    pub fn parse_html_for_line(&mut self, line: String) {
...
pub fn css_for_theme_with_class_style(theme: &Theme, style: ClassStyle) -> Result<String, Error> {
    ...
    css.push_str(&format!(" * theme \"{}\" generated by syntect-patched\n", name));
    css.push_str(" */\n\n");
...
pub enum ClassStyle {
    ...
    /// can ensure that the theme's CSS applies precisely to syntect-patched's
    /// output.
    ///
    ...
    /// separately from the rest of syntect-patched, as it only uses the
    /// public API.)
...
/// and `HighlightLines` from `syntect-patched::easy` to create a full highlighted HTML snippet for
...
/// Convenience method that combines `start_highlighted_html_snippet`, `styled_line_to_highlighted_html`
/// and `HighlightFile` from `syntect-patched::easy` to create a full highlighted HTML snippet for
/// a file.
///
/// Note that the `syntax` passed in must be from a `SyntaxSet` compiled for newline characters.
/// This is easy to get with `SyntaxSet::load_defaults_newlines()`. (Note: this was different before v3.0)
pub fn highlighted_html_for_file<P: AsRef<Path>>(
    ...
    while highlighter.reader.read_line(&mut line)? > 0 {
        {
            let regions = highlighter.highlight_lines.highlight_line(line.clone(), ss)?;
            append_highlighted_html_for_styled_line(
                &regions[..],
                IncludeBackground::IfDifferent(bg),
                &mut output,
...
pub fn line_tokens_to_classed_spans(
    ...
    for &(i, ref op) in ops {
        if i > cur_index {
            span_empty = false;
            write!(s, "{}", Escape(line[cur_index..i].to_string()))?;
            cur_index = i
        }
    ...
    write!(s, "{}", Escape(line[cur_index..line.len()].to_string()))?;
    Ok((s, span_delta))
}
...
pub fn tokens_to_classed_html(line: String,
                              ops: &[(usize, ScopeStackOp)],
                              style: ClassStyle)
                              -> String {
...
/// use syntect-patched::html::{styled_line_to_highlighted_html, IncludeBackground};
///
...
/// ```
pub fn styled_line_to_highlighted_html(v: &[(Style, String)], bg: IncludeBackground) -> Result<String, Error> {
